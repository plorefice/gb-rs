...
impl DebuggerWindow {
    pub fn new(mut id_gen: widget::id::Generator) -> DebuggerWindow {
        DebuggerWindow {
            window_id: id_gen.next(),
        }
    }

    pub fn draw(&self, ui: &mut UiCell, state: &mut EmuState) {
        let window = widget::Canvas::new().floating(true).w_h(450.0, 140.0);

        window.title_bar("Debugger").set(self.window_id, ui);
